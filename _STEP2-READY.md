...
> `_handoff/_payload/drk-website.md`  **and**  `_handoff/drk-website.md`
> (The chat has no access to this repo. These two files carry the full prose/reasoning; the prompts below
> are just the checklist. `_handoff/drk-website.md` is large — that's expected.)
...
> **ATTACH THIS (before Block 1, keep it attached for both blocks):** `_handoff/mari-website.md`
...
> **ATTACH THIS:** `_handoff/drk-seo.md`  (the image-SEO conventions are fully drafted there).
...
backs it. Create references/image-seo.md from the attached _handoff/drk-seo.md: "filenames are for
...
> **ATTACH THIS:** `_handoff/_payload/drk-work-tracker.md`  (the complete spec — paste/attach the WHOLE
...
inventory, sky-deck naming, equipment ✅s; hull colour dropped as a non-issue). `_handoff/mari-core.md`
...
- [ ] Strip the merged items out of `_handoff/*.md` so the backlog doesn't re-accumulate (verify what landed
      first). Leave the Serge-blocked mari-core items and the open-decision items IN the handoffs.
- [ ] Then go to `_RESUME.md` STEP 3 (second audit) and STEP 4 (slim CLAUDE.md).
