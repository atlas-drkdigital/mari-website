...
- **23 images uploaded** from `_website-ready\boat-page` and tagged, via the new
  `_scripts/seed-gallery-images.ts` (re-runnable; `npx sanity exec _scripts/seed-gallery-images.ts
...
3. **Auto-derived alt text on all 23 images.** Generated from Adinda's in-frame descriptors (e.g.
   ...
   `_CONTENT-STATUS.md` yet.
4. **`--color-border-subtle`** (`#ece4d6`) — new token, no objection raised, not explicitly signed off.

## 🟢 THE COPY FOR TOMORROW EXISTS: `_content/_boat.md` (Adinda, 2026-07-17 19:36)

**This is the file I couldn't find earlier — it is `_content/_boat.md`, NOT a section inside
`_CONTENT-STATUS.md`.** Titled "Mari Boat Page — Approved Content Draft", 142 lines, covering: SEO ·
...
is now false). **It's Adinda's call.** Change it in `_scripts/seed-gallery-images.ts`'s
...
- The draft also flags Wi-Fi/Starlink, camera rinse tank, oxygen/first-aid, cert requirements, and the
  ...
  mark it real in `_CONTENT-STATUS.md`.

## 📋 Open / not done

- **`_CONTENT-STATUS.md` "New Boat Content" integration — ADINDA'S ASK, NOT STARTED.** She wrote new
  content under a "New Content Based On" H2 with a `-- Stop Here` line. **I could not find those strings
  in `_CONTENT-STATUS.md`** (`grep` for "New Content Based On", "Stop Here", "New Boat Content" → no
