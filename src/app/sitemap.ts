...
// page (/boats/[slug]) and each destination page (/destinations/[slug]). Blog / other routes are added here as their pages are built, so the
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const { boats = [], destinations = [], privateCharters, about, scheduleRates } = (data ?? {}) as SitemapData
