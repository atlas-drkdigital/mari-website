...
//
// The section still needs an accessible name, so `title` (destinationDefaults.galleryTitle)
// renders as an sr-only h2. `galleryEyebrow` has no consumer on this layout — flagged at build
// time rather than silently rendering chrome the mock doesn't have.
//
...
export function DestinationGallery({
  ...
  return (
    ...
    >
      ...
      <div className="grid w-full grid-cols-2 lg:grid-cols-4" data-reveal>
        {openable.map((img, i) => (
          <button
            key={img._key ?? i}
            type="button"
