...
export function TheBoat({ home }: { home: HomePageData | null }) {
  const eyebrow = home?.theBoatEyebrow ?? 'The Boat'
  const heading = home?.theBoatHeading ?? 'Traditional phinisi liveaboard for serious divers'
  const linkText = home?.theBoatLinkText ?? 'More about the boat'
  return (
      ...
      <div className="mx-auto flex w-full max-w-[1280px] flex-col items-center gap-48 page-gutter-x lg:flex-row lg:gap-64">
        ...
        >
          <Image
            ...
            alt={home?.theBoatImage?.alt ?? 'The Mari phinisi under sail against a clear blue sky'}
            fill
        ...
        <div className="flex flex-1 flex-col gap-24" data-reveal="left">
          ...
          <div className="flex flex-col gap-16 text-body-large text-text-primary">
            {home?.theBoatBody?.length ? (
              <RichText value={home.theBoatBody} />
            ) : (
              <>
