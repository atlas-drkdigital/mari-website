    ...
    <section id="latest-articles" aria-labelledby="latest-articles-heading" className="w-full bg-bg-page pt-80 pb-80 lg:pt-160 lg:pb-160">
      <div className="mx-auto flex w-full max-w-[1400px] flex-col gap-[36px] page-gutter-x lg:gap-64">
        <div data-reveal="left" className="flex flex-col gap-24 lg:gap-32">
          <p className="text-eyebrow uppercase text-action-primary">Our Journal</p>
          <div className="flex flex-col items-start gap-12 lg:flex-row lg:items-center lg:gap-48">
            <h2 id="latest-articles-heading" className="mr-[40px] max-w-[640px] text-display-h2 text-text-primary lg:mr-0">Latest news &amp; deals</h2>
        ...
        >
          {ARTICLES.map((article) => (
            <article key={article.title} className="group/card w-[84%] shrink-0 snap-center lg:w-auto lg:flex-1">
              <a href="#" className="flex flex-col gap-32">
                <div className="relative aspect-[348/362] w-full overflow-hidden">
                  <Image src={article.image} alt={article.alt} fill sizes="(min-width: 1024px) 33vw, 84vw" className="object-cover transition-transform duration-700 ease-in-out group-hover/card:scale-105" />
                </div>
