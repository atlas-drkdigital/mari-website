# _internal/SKILL-ROUND-PLAN.md — paste-and-go skill-update round (prepped 2026-07-20)

This is **STEP 2** of the plan in `_internal/RESUME.md`: the full skill-update round that merges the whole
`_internal/AUDIT-2026-07-20.md` backlog into the skills. It is a **prep deliverable** — Adinda runs each block below
...
- Each block has: **(A)** a ready-to-paste chat prompt (the code block), and **(B)** the exact
  `_internal/handoff` / `_payload` source file(s) to paste alongside it (the chat session has NO access to this repo,
...
- After the last skill: drop the updated skill packages into Downloads; Claude installs them; then
  **strip the merged items out of `_internal/handoff/*.md`** so the backlog doesn't re-accumulate (verify what
  actually landed first).

## ⚠️ Overlap with the 3 URGENT fixes already in `_internal/RESUME.md`
`_internal/RESUME.md` STEP 1 stages three urgent, build-affecting fixes Adinda may already have run in chat. This
...
- `_internal/handoff/_payload/drk-website.md` → **PART E** (Figma-supersede) and **PART F** (stepped-vs-fluid).
- `_internal/handoff/drk-website.md` → the entire **## Pending** section (the ~16 items — full prose bodies, target
...
**Source files to paste alongside the prompt:**
- `_internal/handoff/atlas-website.md` → note the **Pending** items are all HELD-for-Adinda's-decision (do NOT
  resolve them). The two real changes this round are described in the prompt directly.
- Reference: `_internal/AUDIT-2026-07-20.md` → atlas-website lane (C1/S1 and U1).
...
**Source files to paste alongside the prompt:**
- `_internal/handoff/mari-website.md` → the **Pending** section (the SEO-pointer item, the private-charters deck-name
  item, the boat.md image/gallery facts, the `/boat`→`/boats` URL item, the boat.md node/open-item item).
- Reference: `_internal/AUDIT-2026-07-20.md` → mari-website lane.
...
**Source file to paste alongside the prompt:** `_internal/handoff/_payload/drk-work-tracker.md` — it is the canonical,
...
`_internal/handoff/mari-core.md` was deleted — there is nothing to merge. The only follow-through lives in the
...
**Paste alongside:** `_internal/handoff/drk-seo.md` (the image-SEO conventions are fully drafted there).

```
In the drk-seo skill:

1) [HIGH] Add image SEO — it's entirely absent, yet drk-website's vanity-filename chain assumes drk-seo
backs it. Create references/image-seo.md from _internal/handoff/drk-seo.md: "filenames are for ORGANISATION, alt
...
- [ ] Strip merged items out of `_internal/handoff/*.md` (and any MANAGER.md staging) so the backlog doesn't
...
- [ ] Then proceed to `_internal/RESUME.md` STEP 3 (second audit) and STEP 4 (slim CLAUDE.md).
