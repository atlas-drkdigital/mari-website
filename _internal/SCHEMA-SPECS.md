# _internal/SCHEMA-SPECS.md — Sanity page-type specs, tracked for approval
...
currently only holds content checklists, not schema-level field specs) via `_internal/handoff/atlas-website.md`,
...
Supersedes the `faqGeneral` entry in the block below. Full brief: `_internal/handoff/_NEXT-SESSION-faq.md`.
...
- `Faq.tsx` hardcodes `useState('faq-7')` as the default-open item — fine at 8 featured, opens nothing if
  the featured count drops below 8. Pre-existing, logged in `_internal/POLISH-BACKLOG.md`.
...
**Pending review to-do (Adinda, not now):** the whole FAQ remodel (see _internal/QA-CHECKLIST.md).
...
- Sitewide FAQ SEO/AEO conventions queued for **`drk-seo`** (`_internal/handoff/drk-seo.md`) so it's reusable.
...
loaded — see `_internal/CONTENT-STATUS.md`. **This is the one type with a real multi-session review pass behind
...
- [ ] **`galleryTabs[]` — NEW 2026-07-17.** `{category (fixed list), heading, body (richTextBasic)}`,
      ...
      - **Known, ACCEPTED intuitiveness cost:** a tab's copy (here) and its images (`gallery` below) are
        ...
        rethink-next-project item in `_internal/handoff/drk-website.md`, not a mistake to fix here.
...
- ⚠ **The four eyebrows are placeholder pending Adinda's content pass** — Figma has no eyebrow text for
  ...
  sourcing in `_internal/CONTENT-STATUS.md`.
...
- [ ] Editability standard: every heading and eyebrow is editable, site-wide, for SEO — the closed
      ...
      DRK-wide standard (see `_internal/handoff/drk-website.md`), not just a Mari rule.
