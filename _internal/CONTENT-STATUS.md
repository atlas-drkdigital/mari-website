# _internal/CONTENT-STATUS.md — real vs. placeholder content tracker
...
Companion to `_internal/SCHEMA-SPECS.md` (tracks whether a *field* exists and is approved) — this tracks whether
...
| FAQ shared categories | 🟢 | toggle | "Booking Terms" + "What's Included" toggled onto this page — ⚠️ naming vs the mock's "Payment & Booking" flagged in _internal/QA-CHECKLIST.md |
...
Seeded long via `_internal/scripts/seed-overview-body-long.ts` so the Overview's "Read more" could actually be
...
| Field | Status | Source | Note |
|---|---|---|---|
| hero copy (intro/main/sub) | 🟢 | Codex draft | `_internal/content/about-page.md`, approved by Adinda, seeded verbatim |
...
| `seo.ogImage` | 🟢 | **RESOLVED 2026-07-24 — site-wide default** | Was 🔴: this page shape has no hero image, so `buildSeoMetadata` had no `fallbackImage` and /terms served **no `og:image` at all**. Fixed with the better of the two options — a site-wide default (`siteSettings.defaultShareImage`) as the LAST tier of the chain, so Onboard Prices and every future hero-less page are covered too. Verified in served HTML: /terms now emits `og:image` + `twitter:image`; /booking, /about, /private-charters, /boats/mari still emit THEIR OWN hero (the new tier never overrides a more specific one). This doc's own `seo.ogImage` stays deliberately empty — the default is the point. |
...
## `siteSettings` — singleton (doc id `siteSettings`)
Only the fields whose VALUE status is worth tracking are listed; the contact/social details are real client data entered in Studio.
| Field | Status | Source | Note |
|---|---|---|---|
