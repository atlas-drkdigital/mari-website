...
> ⚠️ **RENAME NOTICE (2026-07-24): `_handoff/` is now `_internal/handoff/`.** The root cleanup
> refactor moved every root-level `_*` entry into one `_internal/` folder (e.g. `_RESUME.md` →
> `_internal/RESUME.md`, `_scripts/` → `_internal/scripts/`, `content/` → `_internal/content/`).
> Any chat-side skill text that references `_handoff/` (or other old root `_*` paths) should be
> updated to the `_internal/...` paths at the next skill-update round.
...
**Companion item:** the opt-in `data-reveal` scroll animation bug is queued in `_internal/handoff/atlas-website.md`
...
`download_file_internal/content-scratch` returns **base64 into context** (~350k tokens per MB — not viable for one photo,
...
- **The rule already exists for skills and memories** ("recalled memories reflect what was true when
  ...
  reminders, `_internal/handoff` items, to-dos. A stale reminder is worse than no reminder: it spends the user's
...
- Gallery pattern is marked EXPERIMENTAL in the skill; the real-editor multi-file-drop validation is a Mari
  build task (see `_internal/SCHEMA-SPECS.md`).
