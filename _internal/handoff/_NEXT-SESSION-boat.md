...
1. _internal/handoff/_NEXT-SESSION-boat.md  <- this brief. Hero + Overview are DONE and
...
5. _internal/ADINDA-TODOS.md  <- what's blocked on her (Sanity CORS, the open decisions)
6. every other _internal/handoff/*.md
...
All four now live in **`_internal/ADINDA-TODOS.md` (#11–14)** — that file is the queue for anything only she can do.
...
5. **Sanity CORS** (`_internal/ADINDA-TODOS.md` #9) — one click, hers only. **NOT a rendering bug** (proven); it
...
- **All image polish → one final content pass** (`_internal/POLISH-BACKLOG.md`): the hero grain (1448px smoke-test
...
`_internal/CONTENT-STATUS.md`. ⚠️ **Every gallery image must be tagged with its category** or it vanishes from all
...
- **`_internal/scripts/`, `_internal/handoff/`, `_internal/CONTENT-STATUS.md` are gitignored** (`/_*`, root-anchored). Correct, not a
  bug.
- **PowerShell/bash heredocs mangle quotes** — prefer the Write/Edit tools over `cat <<EOF` for anything
  with backticks or nested quotes. Use `npx sanity exec _internal/scripts/<x>.ts --with-user-token` for GROQ.
...
- **`boat.overviewBody` is TEST CONTENT** (`_internal/scripts/seed-overview-body-long.ts`) — long, with h3/h4/bold/
