...
// content is tracked in _CONTENT-STATUS.md. Idempotent: createOrReplace with fixed IDs + asset
// uploads keyed by originalFilename dedupe. Run:
//   npx sanity exec _scripts/seed-homepage.ts --with-user-token
...
async function run() {
  ...
  const docs: Array<Record<string, unknown>> = [
    ...
    // MUST be removed or replaced with real reviews before launch (see _CONTENT-STATUS.md). Seeded
  ...
  // placeholders ("still being finalized") — tracked in _CONTENT-STATUS.md.
